...
Have you ever wondered how you can go up to a chest in Minecraft and press <kbd>e</kbd> to open it? Or how, when someone dies right next to you, you just pick up all their items by walking into them. Well, wonder no more because I’ll be showing you how those systems work in this project tutorial!
...
Moving on, we have the final bit of code. This is actually really cool because not only do we have an event triggering just for walking into the Area2D, but now we are adding an additional level of interaction to that! Now we are checking for two new conditions, which are the player_inside variable that we have been turning on and off whenever we enter and exit our Area2D, and we are checking for an input. In this case, I’ve set up the input to be the letter “E” being pressed on the keyboard, but you can make it whatever you want! Now, when I go into an object's Area2D and press <kbd>e</kbd>, I get to see a description about that object in the game!
