...
The `--save-dev` flag tells npm these are development tools, meaning they're only needed while building the app and not in the final product. `electron-reload` is especially handy because it automatically refreshes the app every time you save a file, so you're not constantly restarting it manually.s
...
First, close the app with <kbd>ctrl</kbd> + <kbd>c</kbd> (or <kbd>cmd</kbd> + <kbd>C</kbd> on Mac) in your terminal. Then open **main.js** and add these two lines between `center` and `alwaysOnTop` attributes:
...
Challenge yourself to add more advanced features to your application. Here are a few ideas to expand on it:
