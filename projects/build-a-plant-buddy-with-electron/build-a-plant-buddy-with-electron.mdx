---
title: Build a Plant Buddy Desktop App with Electron
author: Sherlyne Hernandez
uid: fLevTT7TaXRMpILsGeIl03pLkNu1
description: Build a tiny desktop app with Electron that mimics a virtual plant you need to water and take care of. 🪴
datePublished: 2026-07-04
published: beta
header: https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fcreate-a-plant-buddy-with-electron%2Fbanner.webp?alt=media
bannerImage: https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fcreate-a-plant-buddy-with-electron%2Fbanner.webp?alt=media
readTime: 60
...
_This project tutorial was selected as the winner of the June 2026: [Project Tutorial Monthly Challenge](https://www.codedex.io/community/monthly-challenge/UXl5qgB24DBLoatPpOWP/winners)._

## Introduction

What if your desktop had its own little [Tamagotchi](https://en.wikipedia.org/wiki/Tamagotchi) or [#30NitesOfCode](https://www.codedex.io/30-nites-of-code), except instead of feeding a digital pet, you're keeping a lil' tiny plant alive? That's exactly what we're building today, a cute ✨desktop app✨ with a virtual plant you need to take care of! 🪴

Here's a quick look at it in action:

<ImageZoom
  src="https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fcreate-a-plant-buddy-with-electron%2Fdemo.webp?alt=media"
  style={{ width: "100%", height: "auto" }}
  alt="Demo"
/>

To build our Plant Buddy we'll be using [Electron](https://www.electronjs.org), a framework that turns familiar web technologies like HTML, CSS, and JavaScript into real, installable desktop apps. We're using it here because it's super beginner-friendly and lets you skip learning a whole new language or stack. If you can build a webpage, you already have what it takes to build this project!

For this project tutorial, we'll be following a simple design I put together in [Figma](https://www.figma.com/community/file/1650050617386672796/plant-buddy), so you'll be styling the app to match it exactly. Feel free to use it as your guide, or tweak the colors, layout, and sprites to make your own version once you're done!
...
We'll only have to restart the app whenever we add something to the **main.js**, since it controls the main process and any changes here need a full restart to take effect, unlike **index.html**, **style.css**, and **script.js**.
...
The `BrowserWindow` is how Electron creates a window on your desktop. Inside it we define how that window looks and behaves. These are called attributes, and here are the most relevant ones: 

- `width` and `height` to set a fixed window size
- `alwaysOnTop` so your plant buddy stays visible on your desktop

The last line tells Electron to wait until the app is ready before creating the window and loading **index.html** into it.
...
<ImageZoom
  src="https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fcreate-a-plant-buddy-with-electron%2Fblank-window.webp?alt=media"
  style={{ width: "100%", height: "auto" }}
  alt="Blank window"
/>
...
<ImageZoom
  src="https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fcreate-a-plant-buddy-with-electron%2Fhtml.webp?alt=media"
  style={{ width: "100%", height: "auto" }}
  alt="HTML"
/>
...
First, close the app in the terminal with:

- Windows: <kbd>ctrl</kbd> + <kbd>c</kbd>
- Mac: <kbd>cmd</kbd> + <kbd>c</kbd>

Then open **main.js** and add these two lines between `center` and `alwaysOnTop` attributes:
...
The `*` reset is a common trick to strip out the browser's built-in spacing so everything starts from a clean slate, and `html` and `body` are set to fill the full window size and hide the content that overflows it. 

The `.app` class adds the green border, the light background color, and sets Pixelify Sans as the font for the whole app.
...
The `.plant-zone` uses a grid layout to stack the sprite and mood tag vertically inside a rounded green bordered box. 

The `image-rendering: pixelated` on `#plant` keeps the sprite looking crisp instead of blurry when scaled up, which is important for pixel art. 

The mood tag is styled as a small rounded pill that sits neatly below the plant.
...
The `.action-btn` class sets the shared layout for both buttons, stacking the name and timer text vertically. 

Each button then gets its own color scheme, the water button is styled in blue and the restart button in yellow, so they're easy to tell apart at a glance. 

The `.timer` text is slightly smaller than the `.name` to create a clear visual hierarchy between the two.

Here’s how the app should look once we applied all the styles:

<ImageZoom
  src="https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fcreate-a-plant-buddy-with-electron%2Fcss.webp?alt=media"
  style={{ width: "100%", height: "auto" }}
  alt="CSS"
/>
...
First, close the app with <kbd>ctrl</kbd> + <kbd>c</kbd> (or <kbd>cmd</kbd> + <kbd>c</kbd> on Mac) in your terminal, same as we did in the CSS. 

Then open **main.js** and add `ipcMain` to your import:
...
Challenge yourself to add more advanced features to your application. Here are a few ideas:
...
Here are some extra resources in case you want to dig deeper or explore the full design:
...
<ImageZoom
  src="https://firebasestorage.googleapis.com/v0/b/codedex-io.appspot.com/o/projects%2Fcreate-a-plant-buddy-with-electron%2Fleafy-stem-thriving.webp?alt=media"
